...
//        gamepadDrivetrain.getGamepadButton(GamepadKeys.Button.B).whenPressed(new hangCommand());
    ...
    public void run(){
        ...
        if(robot.deposit.getPitchState() == depositSubsystem.armState.drop && robot.intake.getState() != intakeSubsystem.intakeState.hang){
            ...
            headingVector.setComponents((gamepadDrivetrain.getTrigger(GamepadKeys.Trigger.LEFT_TRIGGER)-gamepadDrivetrain.getTrigger(GamepadKeys.Trigger.RIGHT_TRIGGER))*0.35, robot.follower.getPose().getHeading());
        }
        else{
